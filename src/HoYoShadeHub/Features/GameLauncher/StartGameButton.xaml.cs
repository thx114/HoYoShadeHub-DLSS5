...
public sealed partial class StartGameButton : UserControl
{
    ...
    /// <summary>注入模式（且勾了 HoYoShade / OpenHoYoShade）：按钮文案改成「启动注入器」（用户要求）</summary>
    public bool IsInjectMode { get; set { if (SetProperty(ref field, value)) UpdateActionButtonState(); } }
