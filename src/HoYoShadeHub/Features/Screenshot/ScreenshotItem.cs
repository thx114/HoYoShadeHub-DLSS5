...
public partial class ScreenshotItem : ObservableObject
{
    ...
    {
        // starward
        if (DateTime.TryParseExact(HoYoShadeHubNameRegex().Match(name).Value, "yyyyMMdd_HHmmssff", null, DateTimeStyles.None, out time))
        {
            return true;
        }
        // genshin zzz
    ...
    private static partial Regex HoYoShadeHubNameRegex();
