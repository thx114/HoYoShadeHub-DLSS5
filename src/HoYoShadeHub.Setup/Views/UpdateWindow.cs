...
public class UpdateWindow : WindowBase
{
    ...
    public UpdateWindow() : base()
    {
        ...
        this.Title = Lang.HoYoShadeHubUpdater;
        BuildUI();
    }
    ...
    private async void StartUpdate()
    {
        try
        {
            ...
            {
                string exe = Path.Combine(installFolder, "HoYoShadeHub.exe");
                if (File.Exists(exe))
                {
        ...
        try
        {
            ...
            if (File.Exists(exe))
            {
                Process.Start("explorer", $"""
                    "{Path.Combine(InstallFolder, "HoYoShadeHub.exe")}"
                    """);
                Environment.Exit(0);
            }
            else
