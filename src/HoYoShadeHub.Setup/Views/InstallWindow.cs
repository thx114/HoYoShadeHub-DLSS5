...
namespace HoYoShadeHub.Setup.Views;
...
public class InstallWindow : WindowBase
{
    ...
    public InstallWindow() : base()
    {
        ...
        this.Title = Lang.HoYoShadeHubSetup;
        BuildUI();
    }
    ...
    private void BuildUI()
    {
        this.Content = new Grid().Rows("1*,1*").Children(
             ...
             new Button().Ref(out Button_Launch)
                         ...
                         .Padding(20, 10, 20, 10)
                         .Center()
                         .Content(Lang.HoYoShadeHubLaunch)
                         .BorderThickness(0)
                         .Row(1)
                         .IsVisible(false)
          );
    ...
    private StackPanel BuildHeader()
    {
        return new StackPanel().Row(0)
            ...
            new Image().Center()
                       ...
                       .SourceResource<InstallWindow>("HoYoShadeHub.Setup.logo.png"),
    ...
    private void SetDefaultInstallFolder()
    {
        try
        {
            ...
            if (!Directory.Exists(folder) || !string.Equals(new DirectoryInfo(folder).Name, "HoYoShade Hub", StringComparison.OrdinalIgnoreCase))
            {
                folder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "HoYoShade Hub");
            }
            SetInstallFolder(folder);
        }
        catch
        {
            SetInstallFolder(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "HoYoShade Hub"));
        }
    }
    ...
    private async Task PrepareManifestAsync(CancellationToken cancellation = default)
    {
        try
        {
            ...
            TextBlock_TitleHeader.Text = $"{Lang.HoYoShadeHubSetup}  ·  {_installService.AppVersion}";
        }
        catch (Exception ex)
        {
            Debug.WriteLine(ex);
    ...
    private async void ChangeInstallFolder()
    {
        try
        {
            ...
            {
                if (new DirectoryInfo(folder).Name is not "HoYoShade Hub")
                {
                    folder = Path.Combine(folder, "HoYoShade Hub");
                }
                SetInstallFolder(folder);
    ...
    private async void StartInstall()
    {
        ...
        try
        {
            if (!string.Equals(new DirectoryInfo(InstallFolder.Value).Name, "HoYoShade Hub", StringComparison.OrdinalIgnoreCase))
            {
                if (SilentMode)
                {
            ...
            await File.WriteAllTextAsync(Path.Combine(InstallFolder.Value, "HoYoShadeHubInstallFolder"), InstallFolder.Value);
            CreateShortcut();
    ...
    private void Launch()
    {
        try
        {
            string exe = Path.Combine(InstallFolder.Value, "HoYoShadeHub.exe");
            if (File.Exists(exe))
            {
                Process.Start("explorer", $"""
                    "{Path.Combine(InstallFolder.Value, "HoYoShadeHub.exe")}"
                    """);
                Environment.Exit(0);
            }
            else
    ...
    {
        string folder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Programs), "HoYoShade Hub");
        Directory.CreateDirectory(folder);
        CreateShortcutInternal(Path.Combine(folder, "HoYoShade Hub.lnk"));
        try
        {
            // 创建桌面快捷方式可能被安全软件拦截
            CreateShortcutInternal(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "HoYoShade Hub.lnk"));
        }
        catch { }
    }
    ...
    private void CreateShortcutInternal(string shortcutPath)
    {
        ...
        {
            Path = Path.Combine(InstallFolder.Value, "HoYoShadeHub.exe"),
    ...
    private void OpenPrivacyPolicy()
    {
        ...
        {
            FileName = "https://hoyosha.de/user-agreement",
