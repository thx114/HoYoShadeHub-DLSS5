...
public class UninstallWindow : WindowBase
{
    ...
    public UninstallWindow() : base()
    {
        ...
        this.Title = Lang.HoYoShadeHubUninstaller;
        BuildUI();
    }
    ...
    private void BuildUI()
    {
        this.Content = new Grid().Columns("2*,3*").Children(
            new TextBlock().Top()
                           .Left()
                           .FontSize(13)
                           .Text(Lang.HoYoShadeHubUninstaller)
    ...
    private async void CheckUninstall()
    {
        try
        {
            ...
            string flag = Path.Combine(currentFolder!, "HoYoShadeHubInstallFolder");
            ...
            bool hasDatabase = files.Any(f => Path.GetFileName(f).Equals("HoYoShadeHubDatabase.db", StringComparison.OrdinalIgnoreCase));
            if (hasDatabase)
            {
                caution = true;
    ...
    {
        string lnk = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "HoYoShade Hub.lnk");
        if (File.Exists(lnk))
        {
            File.Delete(lnk);
        }
