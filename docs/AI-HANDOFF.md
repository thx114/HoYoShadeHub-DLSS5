...
| 东西 | 值 |
| --- | --- |
| 最近 dev 实例 | `build\HoYoShadeHub\app-9.9.30\`（每次 +1，如 9.9.30 → 9.9.31） |
...
- **还没做**：GitHub 更新渠道 + 一键更新 + 退回旧版本（设计见 `OPEN-SOURCE-PLAN.md` §5，需要仓库地址）。
...
| 东西 | 值 |
| --- | --- |
| 最近 dev 版本 | app-9.9.30 |
| 最近便携包 | `HoYoShadeHub_Portable_1.0.21_x64.zip`（914 条目 / 166.5 MB，根目录不带 config.ini） |
