...
| 东西 | 值 |
| --- | --- |
| 最近 dev 实例 | `build\HoYoShadeHub\app-9.9.34\`（每次 +1，如 9.9.34 → 9.9.35） |
...
- **OptiScaler 的 nrdll（1.0.24）**：各分支手册都要求 `nvngx_dlssnr.dll` 放在包旁边；`OptiScalerRuntime.EnsureNrdll`
  从插件目录（DLL 配置装的位置）复制到构建目录，装完自动补一次，卡片上也有「缺 nvngx_dlssnr.dll」+「放入」按钮。
...
| 东西 | 值 |
| --- | --- |
| 最近 dev 版本 | app-9.9.34 |
| 最近便携包 | `HoYoShadeHub_Portable_1.0.24_x64.zip`（v1.0.22/1.0.23 也在 Release 里，可测「退回」） |
